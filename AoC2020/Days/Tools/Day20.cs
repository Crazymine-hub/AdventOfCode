    ...
    public class Day20 : DayBase
    {
            ...
            List<(int, int, int, string, int, string)> uniqueInnerConnections)
        {
            ...
            while (newImg.Count < images.Count)
            {
                ...
                foreach (var connection in uniqueInnerConnections.Where(x => x.Item1 == selImg.ID || x.Item2 == selImg.ID))
                {
        ...
        private void Render(bool stripped)
        {
            Console.Clear();
            int cellCnt = 0;
            foreach (CameraImage img in images)
            {
                ...
                //--Console.CursorTop;
                foreach (string line in GetLines(img.ToString(stripped)))
                {
                    Console.WriteLine();
